...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:param name="singleReportsMode" select="'embedded'"/> <!-- 'external' or 'embedded' -->
    ...
    <xsl:template match="testresult" mode="summary">
        <tr>
           ...
           <td class="light"> <!--  the failing top step, if any -->
            	<b><xsl:value-of select="results/step[result/failed]/parameter[@name='taskName']/@value"/></b>
                <xsl:text> </xsl:text>
    ...
    <xsl:template match="testresult">
        <xsl:choose>
            ...
            <xsl:when test="$singleReportsMode = 'embedded'">
		        ...
		        <p><a href="#overview">Back to Test Report Overview</a></p>
            </xsl:when>
        </xsl:choose>
    </xsl:template>

    <xsl:template match="testresult" mode="external" xmlns:file="java.io.File">
        <xsl:variable name="fileNumber">
            <xsl:number count="*"/>
        </xsl:variable>
        <xsl:variable name="filename">
            <xsl:value-of select="concat($outputDir, '/File', $fileNumber, '.html')"/>
        </xsl:variable>
        ...
        <xsl:choose>
            <xsl:when test="not(file:exists($theFile))">
                <redirect:write file="{$filename}">
                    <!-- <xsl:message>Creating report file <xsl:value-of select="file:getAbsolutePath($theFile)"/></xsl:message>  -->
                    <html>
                        <head>
                            <title>Test results for: <xsl:value-of select="@testspecname"/></title>
                            <link rel="up" title="Test Report Overview" href="{$indexTests}"/>
                            <link rel="first" title="First test Report" href="File2.html"/>
                            <link rel="stylesheet" type="text/css" href="{$resources.dir}/report.css"/>
                            <script type="text/javascript" src="{$resources.dir}/showHide.js"></script>
                            <script type="text/javascript" src="{$resources.dir}/responseBrowser.js"
                                    id="scriptResponseBrowser"></script>
                        ...
                        <body onload="hideAllSubstepsOnPage('expandall.png','collapseall.png')">
                            ...
                            <div style="text-align: center">
                                <xsl:if test="number($fileNumber) > 1">
                                    <a href="{concat('File', number($fileNumber)-1, '.html')}">Previous result</a>
                                    &#160;&#160;&#160;
                                </xsl:if>

                                <a href="{$indexTests}">Back to Test Report Overview</a>
                                &#160;&#160;&#160;
                                <a href="{concat('File', number($fileNumber)+1, '.html')}">Next result</a>
                            </div>
                        </body>
                    </html>
                </redirect:write>
            ...
            <xsl:otherwise>
                <!--
            <xsl:message>Report file <xsl:value-of select="$filename"/> already exists, no need to create again</xsl:message>
         -->
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
