...
public class BenutzerverwaltungTest {
	...
	public void denyConnectionAsNobodyTest() {
		logout(loginCtx);
	...
	public void createBenutzer() throws BenutzergruppeNotFoundException,
		...
		neuerBenutzer = bvProxy.setPassword(neuerBenutzer,
				NEUER_BENUTZER_BENUTZERNAME);
		...
		neuerBenutzer = bvProxy.createBenutzer(neuerBenutzer);
		...
		validBenutzer = bvProxy.setPassword(validBenutzer,
				NEUER_BENUTZER_BENUTZERNAME);
		...
		invalidBenutzer = bvProxy.setPassword(invalidBenutzer,
				NEUER_BENUTZER_BENUTZERNAME);
		...
		assertEquals(neuerBenutzer, validBenutzer);
		assertFalse(neuerBenutzer.equals(invalidBenutzer));
	}
	...
	public void tryChangeRoles() throws BenutzerNotFoundException,
		...
		assertEquals(benutzer.getBenutzergruppen(), alteBenutzergruppen);
	}
	...
	public void changeRolesAsAdmin() throws BenutzerNotFoundException,
		...
		assertEquals(benutzer.getBenutzergruppen(), adminRolleSet);
	}
	...
	@Test
	public void changeOwnPasswordAsUser() throws BenutzerNotFoundException, PasswordNotValidException {
		final Benutzer benutzerVorher = bvProxy.findBenutzer(USERNAME_SACHBEARBEITER);
		bvProxy.setPassword(benutzerVorher, PASSWORTHASH_AENDERN);
		final Benutzer benutzerNachher = bvProxy.findBenutzer(USERNAME_SACHBEARBEITER);
		assertNotNull(benutzerVorher);
		assertNotNull(benutzerNachher);
		
		assertFalse(benutzerVorher.getPasswortHash().equals(benutzerNachher.getPasswortHash()));
		assertTrue(bvProxy.checkPassword(benutzerNachher, PASSWORTHASH_AENDERN));
		
		logout(loginCtx);
		loginCtx = login(LOGIN_DOMAIN_KIPF, USERNAME_SACHBEARBEITER, PASSWORTHASH_AENDERN);
		
		bvProxy.setPassword(benutzerVorher, PASSWORD_SACHBEARBEITER);
	}
	
	@Test
	public void changeUserPasswordAsAdmin() throws BenutzerNotFoundException, PasswordNotValidException {
		logout(loginCtx);
		loginCtx = login(LOGIN_DOMAIN_KIPF, USERNAME_ADMIN, PASSWORD_ADMIN);
